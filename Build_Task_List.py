...
def read_tasks(filename: str = "data/ACORN_DAC_costing_Labor_costs.csv"):
    # read csv file
...
def read_labor_tasks(tasks = [], mapping: dict = {}, filename: str = "data/ACORN_DAC_costing_Labor_costs.csv"):
...
def read_acquisition_tasks(tasks = [], mapping: dict = {}, filename: str = "data/ACORN_DAC_costing_MandS_costs.csv"):
    tasks_dict = {"204.03.02": [],
                  "204.03.03": []}
    for task in tasks:
        if "204.03.02" in task.name:
    ...
    for task in task_list:
        ...
        tasks_dict[task_key][-1].total_cost = (float(task[mapping['unit_price']]) *
                                               float(task[mapping['quantity']]) *
                                               (1 + float(task[mapping['percent_spares']])/100))
        tasks_dict[task_key][-1].duration = float(task[mapping['duration']])
        tasks_dict[task_key][-1].dependencies.extend(task[mapping['dependencies']].split(";"))
...
if __name__ == '__main__':
    ...
    acquisition_mapping = {"name": 0,
                           ...
                           "contingency": 10}
    tasks = []
    read_labor_tasks(tasks,labor_mapping)
    read_acquisition_tasks(tasks,acquisition_mapping)
    export_tasks_to_csv(tasks,"data/test.csv")
